...
void sendData(const char *data, const char *schema){
  ...
  DevKitMQTTClient_Event_AddProp(message, "$$TempProp", f2s(temperature, 1));
...
void showSensors()
{
  try
  {
    ...
    char buff[128];
    sprintf(buff, "CDS Software\r\n \r\nTemp:%s %c    \r\n Hum:%s %c",f2s(temperature, 1),temperatureUnit, f2s(humidity, 1), humidityUnit);
    Screen.print(buff);
